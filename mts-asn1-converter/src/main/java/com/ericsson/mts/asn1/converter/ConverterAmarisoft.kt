...
/**
 * Implementation of [AbstractConverter] for Amarisoft Logs.
 * Amarisoft Logs are very similar to Qcat logs.
 */
class ConverterAmarisoft : AbstractConverter() {
    private val bitsRegex = """\s'([01\s]+)'B""".toRegex()
    private val bitsRegexHex = """\s'([0-9A-F]+)'H""".toRegex()
    ...
    override fun preprocessLine(line: String, identifier: String, indentationLevel: Int): Boolean {
        ...
        // pop stacks when line starts with "}"
        if (line.contains("}")) {
    ...
    override fun parseBitString(
        ...
        while (getIndentationLevel(nextLine) > indentation) {
            read++
    ...
    private fun getBitsValue(line: String, minLength: Int): String? {
        return if (line.contains("'H")) {
            bitsRegexHex.find(line)
                ?.groupValues?.get(1)?.let {
                    BigInteger(it, 16).toString(2).padStart(minLength, '0')
                }
        } else {
            bitsRegex.find(line)?.groupValues?.get(1)?.replace(" ", "")
        }
    }
